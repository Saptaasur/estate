...
export default function Contact({listing}){
  ...
  return (
    <>
    {landlord && (<div className='flex flex-col'>
      <p>Contact <span className='font-semibold'>{landlord.username}</span> for <span>{listing.name.toLowerCase()}</span></p>
      <textarea className='w-full border p-3 rounded-lg' rows="2" value={message} onChange={onChange} name="message" id="message"></textarea>
